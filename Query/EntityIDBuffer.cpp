...
EntityIDBuffer::EntityIDBuffer() {
	...
	min_max[0] = make_pair(UINT_MAX,1);
	min_max[1] = make_pair(UINT_MAX,1);
}
...
Status EntityIDBuffer::mergeBuffer(EntityIDBuffer* XTemp, EntityIDBuffer* XYTemp)
{
	...
	cout<<"in mergeBuffer "<<totalSize<<endl;
	...
	cout<<"in mergeBuffer "<<this->totalSize * sizeof(ID)<<endl;
	...
	if (XTemp->getSize() == 0 && XYTemp->getSize()!=0) {
		cout<<"@buffer "<<buffer<<endl;
		memcpy(buffer, XYTemp->buffer, usedSize * sizeof(ID));
		return OK;
	}

	if (XYTemp->getSize() == 0 && XTemp->getSize()!=0) {
